...
public sealed class SupabaseDocumentRepository : IDataRepository
{
    ...
    public async Task<int> UpdateWhereEqualAsync(string collection, string field, object value, Dictionary<string, object?> data)
    {
        var clean = RemoveNullValues(CloneDictionary(data));
    ...
    public async Task<int> UpdateWhereEqualAndInAsync(
        string collection,
        string equalField,
        object equalValue,
        string inField,
        ...
        Dictionary<string, object?> data)
    {
        var cleanValues = values
            ...
            .ToArray();
        var clean = RemoveNullValues(CloneDictionary(data));
        if (cleanValues.Length == 0 || clean.Count == 0) return 0;
        ...
        cmd.CommandText = """
            ...
            set data = data || @data::jsonb,
                updated_at = now()
            where collection = @collection
              and ((@equal_field = 'id' and id = @equal_value) or (data ->> @equal_field) = @equal_value)
              and ((@in_field = 'id' and id = any(@values)) or (data ->> @in_field) = any(@values));
            """;
        cmd.Parameters.AddWithValue("collection", collection);
        cmd.Parameters.AddWithValue("equal_field", equalField);
        cmd.Parameters.AddWithValue("equal_value", ToSearchText(equalValue));
        cmd.Parameters.AddWithValue("in_field", inField);
        cmd.Parameters.AddWithValue("values", cleanValues);
        cmd.Parameters.AddWithValue("data", ToJson(clean));
        var affected = await cmd.ExecuteNonQueryAsync();
    ...
    public async Task<int> DeleteWhereEqualAndInAsync(
        string collection,
        string equalField,
        object equalValue,
        string inField,
    ...
    public async Task<Dictionary<string, object?>?> GetByIdAsync(string collection, string documentId, bool includeId = true)
    {
        ...
        if (ShouldUseCache(collection)
            && _cache.TryGetValue(key, out Dictionary<string, object?>? cachedDoc))
        {
    ...
    public async Task<List<RankedDocument>> SearchRankedDocumentsAsync(
        string collection,
    ...
    {
        var cleanCollection = (collection ?? string.Empty).Trim();
        var (vectorExpression, collectionLiteral) = cleanCollection switch
        {
            "tours" => ("travelwai_tour_search_vector(data)", "'tours'"),
            "travel_posts" => ("travelwai_post_search_vector(data)", "'travel_posts'"),
            "schedules" => ("travelwai_schedule_search_vector(data)", "'schedules'"),
            _ => throw new ArgumentException("Bộ sưu tập chưa hỗ trợ tìm kiếm xếp hạng.", nameof(collection))
        };
        var cleanAuthorizedUserId = (authorizedUserId ?? string.Empty).Trim();
        if (cleanCollection == "schedules" && string.IsNullOrWhiteSpace(cleanAuthorizedUserId))
            throw new ArgumentException("Cần xác định người dùng để tìm kiếm lịch trình.", nameof(authorizedUserId));
        ...
        if (cleanFields.Count == 0)
            throw new ArgumentException("Cần ít nhất một trường dữ liệu hợp lệ để tìm kiếm.", nameof(fields));

        var cleanQuery = (query ?? string.Empty).Trim();
        var hasQuery = cleanQuery.Length > 0;
        var safeLimit = Math.Clamp(limit, 1, 100);
        var excludedStatuses = (excludedNormalizedStatuses ?? Array.Empty<string>())
        ...
        var jsonProjection = string.Join(", ", cleanFields.Select(field => $"'{field}', data -> '{field}'"));
        var predicates = new List<string> { $"collection = {collectionLiteral}" };
        if (hasQuery) predicates.Add($"{vectorExpression} @@ search_query.terms");
        if (month.HasValue)
        {
            predicates.Add("coalesce(data ->> 'month', '') ~ '^[0-9]+$' and (data ->> 'month')::int = @month");
        }
        if (excludeDeleted)
        {
            predicates.Add("lower(coalesce(data ->> 'is_deleted', data ->> 'isDeleted', 'false')) not in ('true', '1', 'yes')");
        }
        if (excludedStatuses.Length > 0)
        {
            predicates.Add("lower(travelwai_unaccent_immutable(coalesce(data ->> 'status', ''))) <> all(@excluded_statuses)");
        }
        if (cleanCollection == "schedules")
        {
            predicates.Add("""
                (
                    coalesce(data ->> 'user_id', data ->> 'userId', '') = @authorized_user_id
                    or coalesce(data -> 'shared_with_user_ids', data -> 'sharedWithUserIds', '[]'::jsonb) @> @shared_user_probe::jsonb
                )
                """);
        }

        var rankExpression = hasQuery
            ? $"ts_rank_cd({vectorExpression}, search_query.terms, 32)"
            : "0::real";
        var querySource = hasQuery
            ? "cross join (select websearch_to_tsquery('simple', travelwai_unaccent_immutable(@query)) as terms) search_query"
            : "cross join (select null::tsquery as terms) search_query";

        await using var conn = await _dataSource.OpenConnectionAsync();
        await using var cmd = conn.CreateCommand();
        cmd.CommandText = $"""
            select id,
                   jsonb_strip_nulls(jsonb_build_object({jsonProjection}))::text,
        ...
        if (hasQuery) cmd.Parameters.AddWithValue("query", cleanQuery);
        if (month.HasValue) cmd.Parameters.AddWithValue("month", month.Value);
        if (excludedStatuses.Length > 0) cmd.Parameters.AddWithValue("excluded_statuses", excludedStatuses);
        if (cleanCollection == "schedules")
        {
            cmd.Parameters.AddWithValue("authorized_user_id", cleanAuthorizedUserId);
            cmd.Parameters.AddWithValue("shared_user_probe", ToJson(new List<object?> { cleanAuthorizedUserId }));
        }

        var rows = new List<RankedDocument>();
        await using var reader = await cmd.ExecuteReaderAsync();
        while (await reader.ReadAsync())
        {
            var document = FromJson(reader.GetString(1));
            if (includeId) document["id"] = reader.GetString(0);
            var rank = reader.IsDBNull(2)
                ? 0d
                : Convert.ToDouble(reader.GetValue(2), CultureInfo.InvariantCulture);
            rows.Add(new RankedDocument(document, rank));
        }
    ...
    private static TimeSpan GetCacheDuration(string collection)
    {
        return collection switch
        {
            ...
            "users" => TimeSpan.FromSeconds(30),
            "messages" or "conversations" or "friends" or "schedules" or "plans" or "plan_statuses" or "memories" or "shared_memories" or "tours" or "tour_orders" or "travel_posts" => TimeSpan.FromSeconds(15),
            _ => TimeSpan.FromSeconds(30)
        };
    }

    private static bool ShouldUseCache(string collection)
    {
        // Settings such as the site logo must be visible immediately across
        // application instances after an admin updates them.
        return !string.Equals(collection, "site_settings", StringComparison.Ordinal);
    }
