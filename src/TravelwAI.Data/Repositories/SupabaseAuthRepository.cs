...
public sealed class SupabaseAuthRepository : IAuthRepository
{
    ...
    public async Task<Dictionary<string, object?>> SignUpAsync(string email, string password, string username)
    {
        ...
        {
            return Fail($"Mật khẩu phải có ít nhất {MinimumPasswordLength} ký tự.");
        }
    ...
    public async Task<Dictionary<string, object?>> ResetPasswordWithTokenAsync(string email, string resetToken, string newPassword)
    {
        ...
        {
            return Fail($"Mật khẩu mới phải có ít nhất {MinimumPasswordLength} ký tự.");
        }
    ...
    private async Task<Dictionary<string, object?>?> GetUserAsync(string userId)
    {
        ...
        cmd.CommandText = """
            select id, email, username, created_at, last_login_at, role, is_locked, is_protected,
                   is_online, last_seen_at, last_logout_at
            from app_users_auth
            where id = @id
            limit 1;
            """;
        ...
        var storedOnline = reader.GetBoolean(8);
        var lastSeenAt = reader.IsDBNull(9) ? (DateTime?)null : reader.GetDateTime(9).ToUniversalTime();
        var lastLogoutAt = reader.IsDBNull(10) ? (DateTime?)null : reader.GetDateTime(10).ToUniversalTime();
        var isOnline = storedOnline && lastSeenAt.HasValue && lastSeenAt.Value >= DateTime.UtcNow.AddMinutes(-3);
        return new Dictionary<string, object?>
        {
            ...
            ["last_seen_at"] = lastSeenAt?.ToString("O", CultureInfo.InvariantCulture),
            ["lastSeenAt"] = lastSeenAt?.ToString("O", CultureInfo.InvariantCulture),
            ["last_logout_at"] = lastLogoutAt?.ToString("O", CultureInfo.InvariantCulture),
            ["lastLogoutAt"] = lastLogoutAt?.ToString("O", CultureInfo.InvariantCulture)
        };
    }
    ...
    private static string NormalizeStoredRole(string? role)
    {
        ...
        if (string.Equals(value, "Business", StringComparison.OrdinalIgnoreCase)) return "Company";
