...
public sealed class MemoryService : IMemoryService
{
    ...
    public async Task<string?> CreateMemoryAsync(string userId, string memoryName, string description, string province, string createdAt, List<string> sharedEmails, string? photoUrl, List<string>? photoUrls = null, List<string>? videoUrls = null)
    {
        ...
        var cleanVideoUrls = (videoUrls ?? new List<string>())
            .Where(url => !string.IsNullOrWhiteSpace(url))
            .Select(url => url.Trim())
            .Distinct(StringComparer.OrdinalIgnoreCase)
            .ToList();
        var firstPhotoUrl = cleanPhotoUrls.FirstOrDefault();
        var mediaUrls = cleanPhotoUrls.Concat(cleanVideoUrls).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
        var media = cleanPhotoUrls.Select(url => new Dictionary<string, object?> { ["url"] = url, ["type"] = "image", ["contentType"] = "image/*" })
            .Concat(cleanVideoUrls.Select(url => new Dictionary<string, object?> { ["url"] = url, ["type"] = "video", ["contentType"] = "video/*" }))
            .ToList();
