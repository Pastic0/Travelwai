...
public sealed class FriendService : IFriendService
{
    ...
    public async Task<(List<Dictionary<string, object?>> friends, List<Dictionary<string, object?>> pending)> GetFriendsAsync(string userId)
    {
        ...
        foreach (var rel in relations)
        {
            ...
            var item = new Dictionary<string, object?>
            {
                ...
                ["is_online"] = user.GetValueOrDefault("is_online") ?? false,
                ["isOnline"] = user.GetValueOrDefault("isOnline") ?? false,
                ["presence_status"] = user.GetValueOrDefault("presence_status") ?? "offline",
                ["last_seen_at"] = user.GetValueOrDefault("last_seen_at"),
                ["lastSeenAt"] = user.GetValueOrDefault("lastSeenAt"),
                ["status"] = status,
    ...
    public async Task<Dictionary<string, object?>> UpdateFriendStatusAsync(string requestEmail, string recipientId, string action)
    {
        ...
        return new()
        {
            ...
            ["requester_id"] = requesterId,
            ["recipient_id"] = recipientId,
            ["message"] = "Yêu cầu kết bạn đã được từ chối."
        };
    }
