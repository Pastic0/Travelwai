...
public sealed class TravelService : ITravelService
{
    ...
    private static Dictionary<string, object?> NormalizeProvince(string requestedName, Dictionary<string, object?> province)
    {
        ...
        province["name_vi"] = name;
        province["province_name_vi"] = name;
        province["name_en"] = VietnamesePlaceName.ToAscii(name);
        province["province_name_en"] = VietnamesePlaceName.ToAscii(name);
        province["area"] = area;
        province["region"] = region;
        province["subregion"] = region;
        province["classification"] = $"{area} - {region}";
        province["merged_from"] = mergedFrom;
        province["merged_from_en"] = mergedFrom.Select(VietnamesePlaceName.ToAscii).ToList();
        ...
        if (isArchipelago)
        {
            var belongsTo = GetBelongsTo(name);
            province["belongs_to"] = belongsTo;
            province["belongs_to_en"] = VietnamesePlaceName.ToAscii(belongsTo);
    ...
    private static Dictionary<string, object?>? CreateProvinceFallback(string provinceName)
    {
        ...
        return new Dictionary<string, object?>
        {
            ...
            ["name_vi"] = canonical,
            ["province_name_vi"] = canonical,
            ["name_en"] = VietnamesePlaceName.ToAscii(canonical),
            ["province_name_en"] = VietnamesePlaceName.ToAscii(canonical),
            ["area"] = area,
            ["region"] = region,
            ["subregion"] = region,
            ["classification"] = $"{area} - {region}",
            ["merged_from"] = mergedFrom,
            ["merged_from_en"] = mergedFrom.Select(VietnamesePlaceName.ToAscii).ToList(),
            ["natural_area_km2"] = stats.NaturalAreaKm2,
            ["population"] = stats.Population,
            ["is_archipelago"] = IsArchipelago(canonical),
            ["belongs_to"] = GetBelongsTo(canonical),
            ["belongs_to_en"] = VietnamesePlaceName.ToAscii(GetBelongsTo(canonical)),
