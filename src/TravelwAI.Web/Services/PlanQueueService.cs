...
public sealed class PlanQueueService
{
    ...
    public async Task<PlanQueueState> SyncUserAsync(string userId, string? currentAuthRole = null)
    {
        ...
        var isPermanentPlan = Truthy(userDoc.GetValueOrDefault("plan_is_permanent"))
            || Truthy(userDoc.GetValueOrDefault("planIsPermanent"));
        if (isPermanentPlan && (docRole is "VIP" or "Premium" or "Sales" or "Company"))
        {
            var permanentStartedAt = ParseNullableDate(FirstText(userDoc, "plan_started_at", "planStartedAt"));
            await UpdateAuthRoleIfNeededAsync(userId, currentAuthRole, docRole);
            await SavePermanentUserPlanStateAsync(userId, docRole, permanentStartedAt, now);
            return new PlanQueueState(docRole, permanentStartedAt, null, null, null, null, 0);
        }
        ...
        var managedPlanMarker = FirstText(userDoc, "plan_last_order_id", "planLastOrderId");
        var hasManagedPlan = sold.Count > 0
            || (currentAuthRole is "VIP" or "Premium")
            || (docRole is "VIP" or "Premium")
            || !string.IsNullOrWhiteSpace(managedPlanMarker);
        if (!hasManagedPlan)
        {
        ...
        if (targetRole == "Free"
            && (currentAuthRole is "Sales" or "Company")
            && string.IsNullOrWhiteSpace(managedPlanMarker))
        {
    ...
    private async Task SaveUserPlanStateAsync(string userId, string targetRole, PlanQueueItem? active, PlanQueueItem? next, DateTime now)
    {
        await _repo.SetAsync("users", userId, new Dictionary<string, object?>
        {
            ...
            ["planRole"] = targetRole,
            ["plan_started_at"] = PlanDateValue(active?.StartedAt),
            ["planStartedAt"] = PlanDateValue(active?.StartedAt),
            ["plan_expires_at"] = PlanDateValue(active?.ExpiresAt),
            ["planExpiresAt"] = PlanDateValue(active?.ExpiresAt),
            ...
            ["next_plan_started_at"] = PlanDateValue(next?.StartedAt),
            ["nextPlanStartedAt"] = PlanDateValue(next?.StartedAt),
            ["next_plan_expires_at"] = PlanDateValue(next?.ExpiresAt),
            ["nextPlanExpiresAt"] = PlanDateValue(next?.ExpiresAt),
    ...
    private async Task SavePermanentUserPlanStateAsync(string userId, string role, DateTime? startedAt, DateTime now)
    {
        await _repo.SetAsync("users", userId, new Dictionary<string, object?>
        {
            ["id"] = userId,
            ...
            ["plan_role"] = role,
            ["planRole"] = role,
            ["plan_started_at"] = PlanDateValue(startedAt),
            ["planStartedAt"] = PlanDateValue(startedAt),
    ...
    private static List<PlanQueueItem> BuildSoldPlanItems(IEnumerable<Dictionary<string, object?>> orders)
    {
        return orders
            ...
            .Where(item => (item.Role is "VIP" or "Premium" or "Sales" or "Company") && item.ExpiresAt != DateTime.MinValue)
            .ToList();
    }
    ...
    private static object PlanDateValue(DateTime? value) => value.HasValue ? value.Value : string.Empty;
