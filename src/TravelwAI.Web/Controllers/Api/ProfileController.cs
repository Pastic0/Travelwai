...
public sealed class ProfileController : ApiControllerBase
{
    ...
    public async Task<IActionResult> GetProfile()
    {
        ...
        var imageStorage = await _fileStorage.GetUserImageStorageDetailsAsync(current.userId!);
        var imageStoragePayload = BuildImageStoragePayload(imageStorage);
        profile["image_storage"] = imageStoragePayload;
        profile["imageStorage"] = imageStoragePayload;
    ...
    public async Task<IActionResult> UploadProfilePicture(IFormFile profilePic)
    {
        ...
        return Ok(new
        {
            ...
            profilePic = url,
            imageStorage = BuildImageStoragePayload(imageStorage)
        });
    }
    ...
    public async Task<IActionResult> GetImageStorage()
    {
        var current = await CurrentUserAsync();
        if (!current.ok) return current.error!;
        var details = await _fileStorage.GetUserImageStorageDetailsAsync(current.userId!);
        return Ok(new { success = true, imageStorage = BuildImageStoragePayload(details) });
    }

    [HttpDelete("image-storage")]
    public async Task<IActionResult> DeleteUploadedImages()
    {
        var current = await CurrentUserAsync();
        if (!current.ok) return current.error!;

        var result = await _fileStorage.DeleteUserMessageImagesAsync(current.userId!);
        var details = await _fileStorage.GetUserImageStorageDetailsAsync(current.userId!);

        return Ok(new
        {
            success = true,
            message = result.DeletedCount > 0 ? "Đã xóa ảnh tin nhắn" : "Không có ảnh tin nhắn để xóa",
            deletedCount = result.DeletedCount,
            deletedBytes = result.DeletedBytes,
            imageStorage = BuildImageStoragePayload(details)
        });
    }
    ...
    private static object BuildImageStoragePayload(UserImageStorageDetails details)
    {
        var usage = details.Usage;
        return new
        {
            ...
            messageImageBytes = details.MessageImageBytes,
            messageImageCount = details.MessageImageCount
        };
    }
