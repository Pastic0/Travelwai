...
public sealed class SchedulesController : ApiControllerBase
{
    ...
    public async Task<IActionResult> CreateSchedule(ScheduleRequest schedule)
    {
        ...
        if (id is null)
        {
            return StatusCode(500, new { success = false, detail = "Không thể lưu lịch trình" });
        }

        if (string.IsNullOrWhiteSpace(schedule.OldScheduleId))
        {
            var scheduleTitle = string.IsNullOrWhiteSpace(schedule.Title) ? "lịch trình du lịch" : schedule.Title.Trim();
            await _notifications.CreateForUserAsync(
                current.userId!,
                "schedule",
                InAppNotificationService.ScheduleCreatedCategory,
                "Đã lập lịch trình",
                $"Lịch trình {scheduleTitle} đã được tạo thành công.",
                $"/schedule?id={Uri.EscapeDataString(id)}",
                "schedule",
                id,
                "schedule-created");
        }

        return Ok(new { success = true, schedule_id = id, message = "Đã lưu lịch trình" });
    }
