...
public sealed class TourSalesApiController : ApiControllerBase
{
    ...
    public async Task<IActionResult> UploadTourMedia([FromForm] List<IFormFile>? files)
    {
        var access = await RequireTourAccessAsync();
        if (!access.ok) return access.error!;
        ...
        var uploads = (files ?? new List<IFormFile>())
            ...
            .Take(12)
            .ToList();
        if (uploads.Count == 0) return BadRequest(new { success = false, message = "Vui lòng chọn ảnh hoặc video tour." });
        ...
        foreach (var file in uploads)
        {
            ...
            var url = await _fileStorage.SaveFileAsync(file, access.userId!, "tours");
        ...
        return Ok(new { success = true, media, message = "Đã tải tệp tour" });
    }
    ...
    public async Task<IActionResult> CreateTour([FromBody] TourUpsertRequest request)
    {
        ...
        if (request.Price is null or < 0) return BadRequest(new { success = false, message = "Giá tour không hợp lệ." });
        ...
        var media = CleanTourMedia(request.Media, request.Image);
        var imageUrls = media.Where(item => string.Equals(item["type"]?.ToString(), "image", StringComparison.OrdinalIgnoreCase)).Select(item => item["url"]?.ToString() ?? string.Empty).Where(url => !string.IsNullOrWhiteSpace(url)).ToList();
        var videoUrls = media.Where(item => string.Equals(item["type"]?.ToString(), "video", StringComparison.OrdinalIgnoreCase)).Select(item => item["url"]?.ToString() ?? string.Empty).Where(url => !string.IsNullOrWhiteSpace(url)).ToList();
        ...
        var data = new Dictionary<string, object?>
        {
            ...
            ["sold"] = sold,
            ["status"] = status,
            ["image"] = imageUrls.FirstOrDefault() ?? string.Empty,
    ...
    public async Task<IActionResult> UpdateTour(string id, [FromBody] TourUpsertRequest request)
    {
        ...
        {
            return StatusCode(403, new { success = false, message = "Bạn chỉ được sửa tour của mình." });
        }
        ...
        var nextPrice = request.Price ?? GetDecimal(current, "price");
        if (nextPrice < 0) return BadRequest(new { success = false, message = "Giá tour không hợp lệ." });
        ...
        var nextMedia = request.Media is null ? null : CleanTourMedia(request.Media, request.Image);
        var nextImageUrls = nextMedia?.Where(item => string.Equals(item["type"]?.ToString(), "image", StringComparison.OrdinalIgnoreCase)).Select(item => item["url"]?.ToString() ?? string.Empty).Where(url => !string.IsNullOrWhiteSpace(url)).ToList();
        var nextVideoUrls = nextMedia?.Where(item => string.Equals(item["type"]?.ToString(), "video", StringComparison.OrdinalIgnoreCase)).Select(item => item["url"]?.ToString() ?? string.Empty).Where(url => !string.IsNullOrWhiteSpace(url)).ToList();
        var nextMediaUrls = nextMedia?.Select(item => item["url"]?.ToString() ?? string.Empty).Where(url => !string.IsNullOrWhiteSpace(url)).ToList();
        ...
        var data = new Dictionary<string, object?>
        {
            ...
            ["sold"] = nextSold,
            ["status"] = nextStatus,
            ["image"] = nextMedia is null ? Text(current, "image") : (nextImageUrls?.FirstOrDefault() ?? string.Empty),
            ["image_urls"] = nextMedia is null ? current.GetValueOrDefault("image_urls") : nextImageUrls,
            ["imageUrls"] = nextMedia is null ? current.GetValueOrDefault("imageUrls") : nextImageUrls,
            ["video_urls"] = nextMedia is null ? current.GetValueOrDefault("video_urls") : nextVideoUrls,
            ["videoUrls"] = nextMedia is null ? current.GetValueOrDefault("videoUrls") : nextVideoUrls,
            ["media"] = nextMedia is null ? current.GetValueOrDefault("media") : nextMedia,
            ["media_items"] = nextMedia is null ? current.GetValueOrDefault("media_items") : nextMedia,
            ["mediaItems"] = nextMedia is null ? current.GetValueOrDefault("mediaItems") : nextMedia,
            ["media_urls"] = nextMedia is null ? current.GetValueOrDefault("media_urls") : nextMediaUrls,
            ["mediaUrls"] = nextMedia is null ? current.GetValueOrDefault("mediaUrls") : nextMediaUrls,
            ["tour_sales_id"] = ownerId,
            ["tourSalesId"] = ownerId,
            ["created_by"] = ownerId,
            ["createdBy"] = ownerId,
    ...
    public async Task<IActionResult> DeleteTour(string id)
    {
        ...
        if (!CanEditTour(access.role, access.userId, current))
        {
            return StatusCode(403, new { success = false, message = "Bạn chỉ được xóa tour của mình." });
        }
    ...
    private async Task<(bool ok, IActionResult? error, string? role, string? userId, Dictionary<string, object?>? authUser)> RequireTourAccessAsync()
    {
        ...
        if (!IsTourRole(role))
        {
            return (false, StatusCode(403, new { success = false, message = "Chỉ Admin, Sales hoặc Company mới được truy cập trang bán tour." }), role, current.userId, current.authUser);
        }
    ...
    private async Task<string> GetUserRoleAsync(string? userId)
    {
        ...
        if (string.Equals(role, "Business", StringComparison.OrdinalIgnoreCase)) return "Company";
    ...
    private async Task AttachCompanyAmountsAsync(Dictionary<string, object?> order, string? ownerId, string? ownerRole)
    {
        var originalTotal = GetOrderOriginalTotal(order);
    ...
    private async Task AttachTourSalesToOrdersAsync(List<Dictionary<string, object?>> orders, List<Dictionary<string, object?>> tours, string? role, string? currentUserId)
    {
        ...
        foreach (var order in orders)
        {
            ...
            await AttachCompanyAmountsAsync(order, ownerId, resolvedOwnerRole);
        }
    }
    private static List<Dictionary<string, object?>> CleanTourMedia(List<TourMediaItemRequest>? items, string? legacyImage)
    {
        var normalized = new List<Dictionary<string, object?>>();
        foreach (var item in items ?? new List<TourMediaItemRequest>())
        {
            var url = item.Url?.Trim() ?? string.Empty;
            if (string.IsNullOrWhiteSpace(url)) continue;
            var contentType = NormalizeTourMediaContentType(item.ContentType, url);
            normalized.Add(new Dictionary<string, object?>
            {
                ...
                ["type"] = contentType.StartsWith("video/", StringComparison.OrdinalIgnoreCase) ? "video" : "image"
            });
        }
        var legacy = legacyImage?.Trim() ?? string.Empty;
        if (!string.IsNullOrWhiteSpace(legacy) && !normalized.Any(item => string.Equals(item["url"]?.ToString(), legacy, StringComparison.OrdinalIgnoreCase)))
        {
            normalized.Add(new Dictionary<string, object?>
            {
                ["url"] = legacy,
                ["name"] = Path.GetFileName(legacy.Split('?', '#')[0]),
                ["contentType"] = NormalizeTourMediaContentType(null, legacy),
                ["size"] = 0L,
                ["type"] = IsTourVideoUrl(legacy) ? "video" : "image"
            });
        }
        return normalized.GroupBy(item => item["url"]?.ToString() ?? string.Empty, StringComparer.OrdinalIgnoreCase).Select(group => group.First()).Take(12).ToList();
    }

    private static string NormalizeTourMediaContentType(string? contentType, string url)
    {
        var clean = (contentType ?? string.Empty).Trim().ToLowerInvariant();
        if (clean.StartsWith("image/") || clean.StartsWith("video/")) return clean;
        return IsTourVideoUrl(url) ? "video/mp4" : "image/jpeg";
    }

    private static bool IsTourVideoUrl(string? url)
    {
        var extension = Path.GetExtension((url ?? string.Empty).Split('?', '#')[0]);
...
public sealed class TourMediaItemRequest
{
...
public sealed class TourUpsertRequest
{
    ...
    public List<TourMediaItemRequest>? Media { get; set; }
