...
public sealed class PublicToursApiController : ApiControllerBase
{
    ...
    public PublicToursApiController(
        IAuthService authService,
        IDataRepository repo,
        TourOrderAutomation tourAutomation,
    ...
    public async Task<IActionResult> BookTour(string id, [FromBody] PublicTourBookingRequest request)
    {
        ...
        var tourName = tour.GetValueOrDefault("name")?.ToString() ?? "tour du lịch";
        ...
        await _notifications.CreateForUserAsync(
            ...
            "Đã đặt tour",
            $"Bạn đã đặt {quantity} chỗ tour {tourName}. Mã đơn: {safeOrderId}.",
            "/tours",
            "tour-order",
            safeOrderId,
            "tour-booked");
