...
let selectedMemoryMediaFiles = [];
let memoryMediaPreviewObjectUrls = [];
...
function askTravelBotFromProvinceMap34(question) {
  const prompt = question && typeof question === "object"
    ? question
    : String(question || "").replace(/\s+/g, " ").trim();
  if (!prompt || (typeof prompt === "string" && !prompt)) return;


  if (typeof window.TravelwAIOpenChatbot === "function") {
    window.TravelwAIOpenChatbot();
  } else {
    window.dispatchEvent(new CustomEvent("travelwai:open-chatbot"));
  }

  if (typeof window.TravelwAIAskAI === "function") {
    window.TravelwAIAskAI(prompt);
    return;
  }


  window.dispatchEvent(new CustomEvent("travelwai:ask-ai", { detail: { question: prompt } }));
}


function buildProvinceAiQuestion34(provinceName) {
  const name = String(provinceName || "tỉnh/thành này").trim();
  return {
    vi: `Nên đi đâu và trải nghiệm gì ở ${name}?`,
    en: `Where should I go and what should I experience in ${name}?`
  };
}


function buildCalendarAiQuestion34(item) {
  const viName = String(item?.name || "sự kiện này").trim();
  const enName = String(item?.englishName || item?.name || "this event").trim();

  if (item?.type === "holiday") {
    return {
      vi: `${viName} diễn ra khi nào và có ý nghĩa gì?`,
      en: `When does ${enName} take place, and what is its significance?`
    };
  }
  return {
    vi: `${viName} diễn ra khi nào, ở đâu và có gì nổi bật?`,
    en: `When and where does ${enName} take place, and what are its highlights?`
  };
}
...
function isMemoryVideoUrl(url) {
  return /\.(mp4|webm|mov)(?:$|[?#])/i.test(String(url || ""));
}

function normalizeMemoryMediaItem(item) {
  if (!item) return null;
  if (typeof item === "string") {
    const url = normalizeMemoryPhotoUrl(item);
    if (!url) return null;
    const video = isMemoryVideoUrl(url);
    return { url, type: video ? "video" : "image", contentType: video ? "video/mp4" : "image/jpeg" };
  }
  const url = normalizeMemoryPhotoUrl(item.url || item.src || item.path || "");
  if (!url) return null;
  const contentType = String(item.contentType || item.content_type || "").toLowerCase();
  const video = String(item.type || "").toLowerCase() === "video" || contentType.startsWith("video/") || isMemoryVideoUrl(url);
  return { ...item, url, type: video ? "video" : "image", contentType: contentType || (video ? "video/mp4" : "image/jpeg") };
}

function getMemoryMediaItems(memory) {
  const rawMedia = memory?.media || memory?.media_items || memory?.mediaItems;
  let media = [];
  if (Array.isArray(rawMedia)) media = rawMedia.map(normalizeMemoryMediaItem).filter(Boolean);
  else if (typeof rawMedia === "string" && rawMedia.trim()) {
    try { const parsed = JSON.parse(rawMedia); if (Array.isArray(parsed)) media = parsed.map(normalizeMemoryMediaItem).filter(Boolean); } catch (_) {}
  }
  const legacy = [
    ...parseMemoryPhotoListValue(memory?.photo_urls || memory?.photoUrls || memory?.photos),
    ...parseMemoryPhotoListValue(memory?.video_urls || memory?.videoUrls || memory?.videos),
    ...parseMemoryPhotoListValue(memory?.media_urls || memory?.mediaUrls),
    memory?.photo_url || memory?.photoUrl || memory?.image || ""
  ];
  legacy.map(normalizeMemoryMediaItem).filter(Boolean).forEach(item => { if (!media.some(current => current.url === item.url)) media.push(item); });
  return media;
}

function renderMemoryPhotosHtml(memory, wrapperClass = "memory-image") {
  const items = getMemoryMediaItems(memory);
  if (items.length === 0) return "";
  const renderItem = (item, index) => item.type === "video"
    ? `<video preload="metadata" playsinline controls src="${escapeHtml(item.url)}" aria-label="Video kỷ niệm ${index + 1}"></video>`
    : `<img loading="lazy" decoding="async" src="${escapeHtml(item.url)}" alt="Ảnh kỷ niệm ${index + 1}" />`;
  if (items.length === 1) return `<div class="${wrapperClass}">${renderItem(items[0], 0)}</div>`;
  const shown = items.slice(0, 4);
  const remaining = items.length - shown.length;
  return `
    <div class="${wrapperClass} memory-photo-grid memory-photo-count-${Math.min(shown.length, 4)}">
      ${shown.map((item, index) => `<div class="memory-photo-grid-item">${renderItem(item, index)}${remaining > 0 && index === shown.length - 1 ? `<span class="memory-photo-more">+${remaining}</span>` : ""}</div>`).join("")}
    </div>`;
}
...
function getProvinceElementName34(province) {
  if (!province || typeof province.getAttribute !== "function") return "";

  const stableName = (province.getAttribute("data-province-name") || "").trim();
  if (stableName) return stableName;

  const fallbackTitle = (province.getAttribute("title") || "").trim();
  if (fallbackTitle) province.setAttribute("data-province-name", fallbackTitle);
  return fallbackTitle;
}


function protectProvinceMapIdentity34(svgElement) {
  if (!svgElement) return;
  svgElement.querySelectorAll(".province, .province-islet").forEach((province) => {
    getProvinceElementName34(province);
    province.setAttribute("data-no-translate", "");
  });
}
...
function getProvinceFestivalNamesTextForDisplay34(provinceInfo) {
  const provinceName = provinceInfo?.province_name || provinceInfo?.name || "";
  if (!provinceName || typeof window.getFestivalNamesText34 !== "function") return "";
...
function renderProvinceCultureQuickSummary(provinceInfo) {
  ...
  const rows = [
    ...
    ["Lễ hội", getProvinceFestivalNamesTextForDisplay34(provinceInfo)],
...
function renderProvinceCultureSections(provinceInfo) {
  ...
  const cards = PROVINCE_CULTURE_LABELS
    .map(([key, label]) => {
      const isFestivalSection = Array.isArray(key) && key.some((item) => String(item).startsWith("le_hoi"));
      const value = isFestivalSection
        ? getProvinceFestivalNamesTextForDisplay34(provinceInfo)
        : getMergedProvinceCultureText(culture, key);
      if (!value) return "";
...
function getProvinceSearchText(province) {
  const stableName = getProvinceElementName34(province);
  const displayTitle = province?.getAttribute?.("title") || "";
  return normalizeProvinceSearchText([stableName, displayTitle, ...getProvinceAliases(province)].join(" "));
}
...
document.addEventListener("DOMContentLoaded", async function () {
  try {
    ...
    if (svgElement) {
      protectProvinceMapIdentity34(svgElement);
...
function initializeMap() {
  ...
  const resetSelectedProvinceStyle = () => {
    ...
    const provinceTitle = getProvinceElementName34(selectedProvince);
  ...
  const selectProvince = (province, options = {}) => {
    ...
    const provinceName = getProvinceElementName34(province) || "Không rõ tỉnh/thành";
    ...
    if (options.scrollToMap === true && window.matchMedia("(max-width: 768px)").matches) {
      window.requestAnimationFrame(() => {
        document.querySelector(".province-layout .map-info-container")?.scrollIntoView({
          behavior: "smooth",
          block: "start"
        });
      });
  ...
  const openProvinceFromSearch = (province) => selectProvince(province, { zoom: true, scrollToMap: true });
  const openProvinceFromList = (province) => selectProvince(province, { zoom: true, scrollToMap: true });
  setupSearch(provinces, openProvinceFromSearch);
...
function renderProvinceQuickList(provinces, onProvinceFound) {
  ...
  const provinceArray = Array.from(provinces).map((province) => {
    const name = getProvinceElementName34(province) || "Không rõ";
  ...
  window.TravelwAILanguage?.translate?.(listEl);
...
function setupSearch(provinces, onProvinceFound) {
  ...
  const applySearch = () => {
    ...
    provinces.forEach((province) => {
      ...
      const provinceName = getProvinceElementName34(province);
...
function showProvinceLoadingPanel(provinceName) {
  ...
  infoPanel.innerHTML = `
    <div class="province-info-header">
      ...
      <button type="button" class="close-info-panel" aria-label="Đóng bảng thông tin"><span data-interface-icon="x"></span></button>
    </div>
...
function showInfoPanel(provinceInfo) {
  ...
  infoPanel.querySelector(".province-ask-ai-btn")?.addEventListener("click", function () {
    askTravelBotFromProvinceMap34(buildProvinceAiQuestion34(provinceName));
  });
  const sidePanel = getProvinceSidePanel();
  if (sidePanel) sidePanel.appendChild(infoPanel);
...
function create_province_detail_Panel(provinceInfo) {
  ...
  detailPanel.innerHTML = `
    <div class="memory-panel-header">
      ...
      <button type="button" class="close-detail-panel close-info-panel" aria-label="Đóng xem chi tiết"><span data-interface-icon="x"></span></button>
    </div>
...
function showMemoryPanel(provinceInfo) {
  ...
  emailsToShareList = [];
  memoryMediaPreviewObjectUrls.splice(0).forEach((url) => URL.revokeObjectURL(url));
  selectedMemoryMediaFiles = [];

  memoryPanel.innerHTML = `
    <div class="memory-panel-header">
      <h3>Thêm kỷ niệm cho ${provinceInfo.name || provinceInfo.province_name}</h3>
      <button type="button" class="close-memory-panel close-detail-panel close-info-panel" aria-label="Đóng thêm kỷ niệm"><span data-interface-icon="x"></span></button>
    </div>
    <div class="memory-panel-content">
      <form id="memoryForm" class="memory-form memory-detail-form">
        ...
        <div class="detail-section memory-form-section">
          <h4>Ảnh và video</h4>
          <div class="form-group">
            <label for="memoryPhoto">Đính kèm tệp</label>
            <div class="photo-upload-area">
              <input type="file" id="memoryPhoto" name="memoryPhotos" accept="image/*,video/*" multiple style="display: none;">
              <div class="upload-placeholder" onclick="document.getElementById('memoryPhoto').click()">
                <div class="upload-icon" data-interface-icon="paperclip"></div>
                <p>Bấm để chọn ảnh hoặc video</p>
                <span class="upload-hint">Mỗi tệp tối đa 10MB</span>
              </div>
...
function handlePhotoUpload(event) {
  ...
  for (const file of files) {
    ...
    if (file.size > 10 * 1024 * 1024) {
      window.TravelwAIToast(`${file.name} phải nhỏ hơn 10MB`);
      continue;
    }
    if (!type.startsWith("image/") && !type.startsWith("video/")) {
      window.TravelwAIToast(`${file.name} không phải ảnh hoặc video`);
      continue;
    }
    selectedMemoryMediaFiles.push(file);
  }
  selectedMemoryMediaFiles = selectedMemoryMediaFiles.slice(0, 12);
  event.target.value = "";
...
function renderMemoryPhotoPreviews() {
  ...
  memoryMediaPreviewObjectUrls.splice(0).forEach((url) => URL.revokeObjectURL(url));
  if (!selectedMemoryMediaFiles.length) {
    previewGrid.innerHTML = "";
    previewGrid.style.display = "none";
    return;
  }
  previewGrid.style.display = "grid";
  previewGrid.innerHTML = selectedMemoryMediaFiles.map((file, index) => {
    const url = URL.createObjectURL(file);
    memoryMediaPreviewObjectUrls.push(url);
    ...
    return `<div class="photo-preview-item">${preview}<button type="button" class="remove-photo" onclick="removeMemoryPhoto(${index})" aria-label="Xóa tệp ${index + 1}"><span data-interface-icon="trash-2"></span></button></div>`;
  }).join("");
}

function removeMemoryPhoto(index) {
  selectedMemoryMediaFiles.splice(index, 1);
  renderMemoryPhotoPreviews();
}
...
async function handleMemorySubmission(provinceInfo) {
  ...
  let memoryFiles = selectedMemoryMediaFiles.slice();
  ...
  if (window.TravelwAIImageOptimizer && memoryFiles.length) {
    const prepared = [];
    for (const file of memoryFiles) prepared.push(file.type.startsWith("image/") ? await window.TravelwAIImageOptimizer.optimizeImageFile(file) : file);
    memoryFiles = prepared;
  }
  ...
  memoryFiles.forEach((file) => formData.append("files", file, file.name));
  ...
  modal.innerHTML = `
    <div class="modal-content">
      <div class="modal-header">
        ...
        <button type="button" class="close-modal-btn" aria-label="Đóng nhật ký"><span data-interface-icon="x"></span></button>
      </div>
...
function renderProvinceCalendarItem34(item) {
  if (typeof item === "string") return `<li><span class="festival-calendar-item-main">${escapeHtml(item)}</span></li>`;

  const line = escapeHtml(item?.line || item?.name || "");
  const name = String(item?.name || "").trim();
  const type = item?.type === "holiday" ? "holiday" : "festival";
  const provinces = Array.isArray(item?.provinces) ? item.provinces.filter(Boolean) : [];
  const askAiButton = name
    ? `<button type="button" class="festival-calendar-ask-ai-btn twai-ai-icon-button" data-travelwai-ask-ai="true" data-ai-event-name="${escapeHtml(name)}" data-ai-event-type="${escapeHtml(type)}" aria-label="Hỏi AI về ${escapeHtml(name)}" title="Hỏi AI về ${escapeHtml(name)}"><span data-interface-icon="sparkles"></span></button>`
    : "";

  return `
    <li>
      <div class="festival-calendar-item-content">
        <span class="festival-calendar-item-main">${line}</span>
        ${provinces.length ? `<span class="festival-calendar-item-provinces">Tỉnh: ${escapeHtml(provinces.join(", "))}</span>` : ""}
      </div>
      ${askAiButton}
    </li>
  `;
  ...
  modal.innerHTML = `
    <div class="modal-content">
      <div class="modal-header">
        ...
        <button type="button" class="close-modal-btn" aria-label="Đóng lịch"><span data-interface-icon="x"></span></button>
      </div>
      <div class="modal-body">
...
function bindFestivalCalendarModalEvents34(modal) {
  ...
  modal.querySelectorAll(".festival-calendar-ask-ai-btn").forEach((button) => {
    button.addEventListener("click", function () {
      const item = {
        name: button.dataset.aiEventName || "",
        type: button.dataset.aiEventType || "festival"
      };
      modal.remove();
      askTravelBotFromProvinceMap34(buildCalendarAiQuestion34(item));
    });
  });
}
...
function renderMemoryJournalModal(modal, memories, errorMessage = "") {
  ...
  const cards = memoryList
    .map((memory) => {
      ...
      const photoHtml = renderMemoryPhotosHtml(memory, "memory-image");
        ...
        <article class="memory-card memory-journal-card memory-item" data-memory-id="${escapeHtml(memoryId)}" data-memory-province="${escapeHtml(province)}" data-memory-collection="${escapeHtml(memoryCollection)}">
          ...
          <div class="memory-details">
            <div class="memory-card-header">
              <h4 data-no-translate>${escapeHtml(title)}</h4>
            </div>
            ...
            <div class="memory-journal-card-footer">
              ...
              ${memoryId ? `<button type="button" class="delete-memory-btn memory-journal-delete-btn" data-memory-id="${escapeHtml(memoryId)}" data-memory-province="${escapeHtml(province)}" data-memory-collection="${escapeHtml(memoryCollection)}" aria-label="Xóa kỷ niệm">Xóa kỷ niệm</button>` : ""}
            </div>
          </div>
        </article>
      `;
    ...
    <div class="memories-list">
      ${memories
        .map((memory) => {
          ...
          const photoHtml = renderMemoryPhotosHtml(memory, "memory-photo");
        ...
        <div class="memory-item" data-memory-id="${escapeHtml(memoryId)}" data-memory-province="${escapeHtml(memory.province || "")}" data-memory-collection="${escapeHtml(memoryCollection)}">
          <div class="memory-content">
            ...
            <p data-no-translate>${escapeHtml(shortDescription)}</p>
...
function renderSharedEmailsList() {
  ...
  emailsToShareList.forEach((item, index) => {
    ...
    removeButton.innerHTML = window.TravelwAIIcons?.html ? window.TravelwAIIcons.html("x") : '<span data-interface-icon="x"></span>';
