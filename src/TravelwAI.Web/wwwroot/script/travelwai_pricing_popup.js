(function () {
  ...
  async function loadPlans(force = false) {
    if (force) planCache = null;
    if (planCache) return planCache;
    if (loadingPlans) return loadingPlans;
    loadingPlans = fetch(`/api/account-plans?_=${Date.now()}`, { cache: "no-store" })
  ...
  function renderPlans(plans) {
    ...
    grid.innerHTML = plans.map((plan) => {
      ...
      const actionHtml = role === "sales" || role === "company"
      ...
      return `
        <article class="twai-price-card twai-price-card-${escapeHtml(role)} ${selected ? "is-featured" : ""}">
          ${selected ? `<span class="twai-price-ribbon">PHỔ BIẾN</span>` : ""}
    ...
    modal.innerHTML = `
      <form class="form-card business-application-card" id="businessApplicationForm">
        <div class="tour-offer-modal-header business-application-header">
          ...
          <button type="button" class="tour-offer-close" data-business-application-close aria-label="Đóng"><span data-interface-icon="x"></span></button>
        </div>
  ...
  function showFreeFeaturePopup(reason) {
...
document.addEventListener("click", function (event) {
  ...
  window.addEventListener("travelwai:account-plans-updated", () => {
    planCache = null;
    if (document.getElementById("travelwaiPricingPopup")?.classList.contains("open")) loadPlans(true).then(renderPlans);
  });
  window.addEventListener("storage", event => {
    if (event.key !== "travelwai-account-plans-updated-at") return;
    planCache = null;
    if (document.getElementById("travelwaiPricingPopup")?.classList.contains("open")) loadPlans(true).then(renderPlans);
  });
  ...
  window.TravelwAIPricingPopup = {
    ...
    reload: async function () {
      const plans = await loadPlans(true);
      if (document.getElementById("travelwaiPricingPopup")?.classList.contains("open")) renderPlans(plans);
      return plans;
    }
  };
})();
