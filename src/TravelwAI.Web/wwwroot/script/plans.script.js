...
function getPlanToken() {
  return localStorage.getItem("idToken") || sessionStorage.getItem("idToken") || getPlanCookie("TravelwAIAuth") || "";
}
...
function renderPlanStatusChoices() {
  ...
  container.innerHTML = planStatusOptions
    .map((status) => {
      ...
      return `
        <button class="plan-status-choice${active}" type="button" role="option" aria-label="${label}" title="${label}" data-key="${escapeAttr(key)}" data-translate-attrs="data-label" data-label="${label}" aria-selected="${active ? "true" : "false"}" style="--plan-accent:${accent}" onclick="savePlanStatus('${escapeAttr(key)}')">
...
function renderPlanCard(plan) {
  const title = renderPlanUserText(plan.title, "Kế hoạch du lịch");
  const destination = renderPlanUserText(plan.destination_display || plan.province_name || plan.destination_status, "Chưa có điểm đến");
  ...
  const ownerName = renderPlanUserText(plan.owner?.username || plan.owner_name, "Người lập kế hoạch");
  ...
  const memberChips = members.length
    ? members.map((member) => `<span class="member-chip" data-no-translate>${escapeHtml(member.username || member.name || "Người dùng")}</span>`).join("")
    : '<span class="member-chip">Chưa có thành viên</span>';
  const tagChips = tags.length ? `<div class="plan-tags">${tags.map(tag => `<span class="plan-tag-chip" data-no-translate style="${getTagAccentStyle(tag)}">${escapeHtml(tag)}</span>`).join("")}</div>` : "";
  ...
  return `
    <article class="schedule-card plan-card">
      ...
      <div class="plan-meta">
        <span><i data-interface-icon="user"></i> Người lập: ${ownerName}</span>
        <span><i data-interface-icon="calendar-days"></i> ${start} - ${end}</span>
        <span><i data-interface-icon="target"></i> Cần ${target} người</span>
      </div>
...
async function handlePlanSubmit(event) {
  ...
  try {
    ...
    closePlanModal();
    showPlanToast("Đã lập kế hoạch.", "success");
    await loadPlans();
  } catch (error) {
    showPlanToast(error.message, "error");
  } finally {
  ...
  container.innerHTML = users
    .map((user) => `
      <div class="plan-search-result" onclick="selectPlanUser('${escapeAttr(user.id || "")}')">
        <div>
          <strong data-no-translate>${escapeHtml(user.username || user.name || "Người dùng")}</strong>
  ...
  list.innerHTML = selectedInviteUsers
    .map((user) => `
      <span class="invited-chip">
        <span data-no-translate>${escapeHtml(user.username || user.name || user.email || "Người dùng")}</span>
        <small data-no-translate>${escapeHtml(user.email || "")}</small>
...
function renderPlanUserText(value, fallback = "") {
