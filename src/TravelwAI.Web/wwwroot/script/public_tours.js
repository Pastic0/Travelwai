...
function publicTourIsVideoUrl(url) {
  return /\.(mp4|webm|mov)(?:$|[?#])/i.test(String(url || ""));
}

function normalizePublicTourMedia(item) {
  if (!item) return null;
  if (typeof item === "string") {
    const url = item.trim();
    if (!url) return null;
    const video = publicTourIsVideoUrl(url);
  ...
  const video = String(item.type || "").toLowerCase() === "video" || contentType.startsWith("video/") || publicTourIsVideoUrl(url);
  return { ...item, url, type: video ? "video" : "image", contentType: contentType || (video ? "video/mp4" : "image/jpeg") };
}

function getPublicTourMedia(tour) {
  const raw = getValue(tour, "media", "media_items", "mediaItems");
  let media = [];
  if (Array.isArray(raw)) media = raw.map(normalizePublicTourMedia).filter(Boolean);
  else if (typeof raw === "string" && raw.trim()) {
    try { const parsed = JSON.parse(raw); if (Array.isArray(parsed)) media = parsed.map(normalizePublicTourMedia).filter(Boolean); } catch (_) {}
  }
  const legacy = [getValue(tour, "media_urls", "mediaUrls"), getValue(tour, "image_urls", "imageUrls", "images"), getValue(tour, "video_urls", "videoUrls"), getValue(tour, "image", "image_url", "imageUrl")];
  legacy.forEach(value => {
    const list = Array.isArray(value) ? value : (typeof value === "string" ? value.split(/[\n,|]+/) : []);
    list.map(normalizePublicTourMedia).filter(Boolean).forEach(item => { if (!media.some(current => current.url === item.url)) media.push(item); });
  });
  return media;
}

function renderPublicTourMedia(tour, className, controls = false) {
  const item = getPublicTourMedia(tour)[0];
  if (!item) return "•";
  if (item.type === "video") return `<video class="${className}" preload="metadata" playsinline ${controls ? "controls" : "muted loop"} src="${escapeHtml(item.url)}"></video>`;
  return `<img class="${className}" loading="lazy" decoding="async" src="${escapeHtml(item.url)}" alt="${escapeHtml(tour?.name || "Tour")}" />`;
}
  ...
  grid.innerHTML = tours.map(t => {
    ...
    return `
      <article class="public-tour-card">
        <div class="public-tour-image">${renderPublicTourMedia(t, "public-tour-media")}</div>
...
function renderPublicBookTourDetails(tour) {
  ...
  box.innerHTML = `
    <div class="public-book-tour-thumb">
      ${renderPublicTourMedia(tour, "public-book-tour-media", true)}
    </div>
