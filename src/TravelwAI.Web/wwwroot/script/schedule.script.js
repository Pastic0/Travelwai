...
let scheduleAiAbortController = null;
...
function canUseScheduleAi() {
  const role = normalizeScheduleAccountRole();
  return role !== "free" && role !== "vip";
}

function showScheduleAiLimit() {
  const message = "AI lập lịch trình chỉ dành cho tài khoản Premium, Admin, Sales hoặc Business.";
  if (window.TravelwAIPricingPopup?.showFreeFeaturePopup || window.TravelwAIPricingPopup?.open) {
    (window.TravelwAIPricingPopup.showFreeFeaturePopup || window.TravelwAIPricingPopup.open)(message);
    return;
  }
  showError(message);
}
...
function showScheduleAccountLimit() {
  if (window.TravelwAIPricingPopup?.showFreeFeaturePopup || window.TravelwAIPricingPopup?.open) {
    (window.TravelwAIPricingPopup.showFreeFeaturePopup || window.TravelwAIPricingPopup.open)("Tài khoản Free chưa dùng được lịch trình. Vui lòng nâng cấp gói.");
    return;
  }
...
async function get_user_friendList() {
  const response = await authenticatedFetch(`${API_BASE}/friend_requests`, {
    method: "GET"
  });
...
function setupEventListeners() {
  ...
  const scheduleAiApplyBtn = document.getElementById("scheduleAiApplyBtn");
  if (scheduleAiApplyBtn) {
    scheduleAiApplyBtn.addEventListener("click", applyScheduleAiInstruction);
    if (!canUseScheduleAi()) {
      scheduleAiApplyBtn.setAttribute("aria-disabled", "true");
      scheduleAiApplyBtn.title = "AI lập lịch trình yêu cầu Premium, Admin, Sales hoặc Business";
    }
  }

  const scheduleAiPrompt = document.getElementById("scheduleAiPrompt");
  if (scheduleAiPrompt) {
    scheduleAiPrompt.addEventListener("keydown", (event) => {
      if (event.key === "Enter" && (event.ctrlKey || event.metaKey)) {
        event.preventDefault();
        applyScheduleAiInstruction();
      }
    });
  }
...
function buildCurrentItineraryAiDraft() {
  const sortedDays = Object.keys(dailyPlans)
    .sort((left, right) => new Date(left) - new Date(right))
    .map((date) => {
      const day = dailyPlans[date] || {};
      return {
        date,
        timePhases: (day.timePhases || []).map((phase) => ({
          name: String(phase.name || "").trim(),
          timeRange: String(phase.timeRange || "").trim(),
...
function setScheduleAiLoading(isLoading) {
  const button = document.getElementById("scheduleAiApplyBtn");
  const prompt = document.getElementById("scheduleAiPrompt");
  if (button) {
    ...
    button.setAttribute("aria-label", isLoading ? "Dừng AI lập lịch trình" : "Tạo hoặc chỉnh lịch trình bằng AI");
    button.title = isLoading ? "Bấm để dừng AI" : "Tạo hoặc chỉnh lịch trình bằng AI";
  }
  if (prompt) prompt.disabled = Boolean(isLoading);
}

function readItineraryAiValue(source, camelName, pascalName) {
  if (!source || typeof source !== "object") return undefined;
  return source[camelName] !== undefined ? source[camelName] : source[pascalName];
}

function applyItineraryAiDraft(draft) {
  if (!draft || typeof draft !== "object") {
    throw new Error("AI trả về lịch trình không hợp lệ.");
  }

  const title = readItineraryAiValue(draft, "title", "Title");
  const description = readItineraryAiValue(draft, "description", "Description");
  const startDate = readItineraryAiValue(draft, "startDate", "StartDate");
  const endDate = readItineraryAiValue(draft, "endDate", "EndDate");
  const budget = readItineraryAiValue(draft, "budget", "Budget");
  const currency = readItineraryAiValue(draft, "currency", "Currency");
  const tags = readItineraryAiValue(draft, "tags", "Tags");
  const days = readItineraryAiValue(draft, "days", "Days");

  if (typeof title === "string") document.getElementById("scheduleTitle").value = title;
  if (typeof description === "string") document.getElementById("scheduleDescription").value = description;
  if (typeof startDate === "string") document.getElementById("startDate").value = startDate;
  if (typeof endDate === "string") document.getElementById("endDate").value = endDate;
  if (budget === null || budget === undefined || budget === "") {
    document.getElementById("budget").value = "";
  } else if (Number.isFinite(Number(budget)) && Number(budget) >= 0) {
    document.getElementById("budget").value = String(Number(budget));
  }
  if (["VND", "USD", "EUR"].includes(String(currency || "").toUpperCase())) {
    document.getElementById("currency").value = String(currency).toUpperCase();
  }
  if (Array.isArray(tags)) {
    document.getElementById("tags").value = tags.map((tag) => String(tag || "").trim()).filter(Boolean).join(", ");
  }

  dailyPlans = {};
  (Array.isArray(days) ? days : []).forEach((day) => {
    const date = readItineraryAiValue(day, "date", "Date");
    if (!/^\d{4}-\d{2}-\d{2}$/.test(String(date || ""))) return;

    const phases = readItineraryAiValue(day, "timePhases", "TimePhases");
    dailyPlans[date] = {
      timePhases: (Array.isArray(phases) ? phases : []).map((phase) => {
        const activities = readItineraryAiValue(phase, "activities", "Activities");
        return {
          name: String(readItineraryAiValue(phase, "name", "Name") || "").trim(),
          timeRange: String(readItineraryAiValue(phase, "timeRange", "TimeRange") || "").trim(),
          activities: (Array.isArray(activities) ? activities : []).map((activity) => ({
            name: String(readItineraryAiValue(activity, "name", "Name") || "").trim(),
            notes: String(readItineraryAiValue(activity, "notes", "Notes") || "").trim(),
...
async function applyScheduleAiInstruction() {
  if (!canUseScheduleAi()) {
    showScheduleAiLimit();
    return;
  }

  const promptInput = document.getElementById("scheduleAiPrompt");
  const instruction = promptInput?.value.trim() || "";
  if (!instruction) {
    showError("Vui lòng nhập yêu cầu cho AI.");
    promptInput?.focus();
    return;
  }

  if (scheduleAiAbortController) {
    scheduleAiAbortController.abort("user-cancelled");
    return;
  }

  const abortController = new AbortController();
  scheduleAiAbortController = abortController;
  ...
  try {
    const response = await authenticatedFetch(`${API_BASE}/itinerary-ai/generate`, {
      method: "POST",
      timeoutMs: 180000,
      body: JSON.stringify({
        instruction,
        language: window.TravelwAILanguage?.get?.() || "vi",
        currentDraft: buildCurrentItineraryAiDraft(),
      }),
    ...
    if (!response.ok || !result.success || !result.data) {
      throw new Error(result.message || "AI chưa cập nhật được lịch trình.");
    }

    applyItineraryAiDraft(result.data);
    promptInput.value = "";
    showSuccess("Đã cập nhật lịch trình bằng AI.");
  } catch (error) {
    if (abortController.signal.aborted) {
      showSuccess("Đã dừng AI lập lịch trình.");
    } else {
      console.error("Lỗi AI lập lịch trình:", error);
      showError(error.message || "AI chưa cập nhật được lịch trình.");
    }
  } finally {
    if (scheduleAiAbortController === abortController) scheduleAiAbortController = null;
      ...
      return `
        <div class="schedule-card ${schedule.shareType === "shared" ? "shared-schedule" : ""}" onclick="openScheduleDetail('${schedule.id}')">
          <div class="schedule-card-top">
            <h3>${renderScheduleUserText(schedule.title, "Chưa đặt tên")}</h3>
            ${badge}
          </div>
          ...
          <div class="tags">
            ${(schedule.tags || []).map((tag) => `<span class="tag" data-no-translate>${escapeHtml(tag)}</span>`).join("")}
          </div>
        </div>
      `;
    })
...
function displayScheduleDetail(schedule, isOwner = true) {
  ...
  detailTitle.setAttribute("data-no-translate", "");
  detailTitle.textContent = schedule.title;
  ...
  detailsContainer.innerHTML = `
    <div class="schedule-detail-content-enhanced">
      ...
      <div class="detail-header-section">
        <h2 data-no-translate>${escapeHtml(schedule.title)}</h2>
      </div>
            ...
            schedule.tags && schedule.tags.length > 0
              ? `<div class="detail-item-enhanced full-width">
                ...
                <span class="tags-list">${(schedule.tags || [])
                  .map(
                    (tag) => `<span class="tag-badge" data-no-translate>${escapeHtml(tag)}</span>`
                  )
                  .join(" ")}</span>
              </div>`
              : ""
          ...
          schedule.days && schedule.days.length > 0
            ...
            <div class="detail-day-card">
              ...
              <div class="day-card-body">
                ${
                  day.notes
                    ? `<div class="day-card-meta notes"><p data-no-translate>${escapeHtml(
                        day.notes
                      )}</p></div>`
                    : ""
                }
                ${
                  day.accommodation
                    ? `<div class="day-card-meta accommodation"><label>Nơi ở:</label> <p data-no-translate>${escapeHtml(
                        day.accommodation
                      )}</p></div>`
                    : ""
                }
                ${
                  day.transportation
                    ? `<div class="day-card-meta transportation"><label>Di chuyển:</label> <p data-no-translate>${escapeHtml(
                        day.transportation
                      )}</p></div>`
                    : ""
                }
...
function closeModal() {
  scheduleAiAbortController?.abort("modal-closed");
  scheduleAiAbortController = null;
  setScheduleAiLoading(false);
  const modal = document.getElementById("scheduleModal");
...
function clearForm() {
  ...
  updateEmailList();
  const scheduleAiPrompt = document.getElementById("scheduleAiPrompt");
  if (scheduleAiPrompt) scheduleAiPrompt.value = "";
  setScheduleAiLoading(false);
...
function renderScheduleUserText(value, fallback = "") {
  const source = String(value ?? "").trim();
  return source
    ? `<span data-no-translate>${escapeHtml(source)}</span>`
    : escapeHtml(fallback);
}
  ...
  users.forEach((user) => {
    ...
    if (user.profilePic) {
      ...
      if (!picUrl.startsWith("http") && typeof API_BASE !== "undefined") {
        ...
        picUrl = "/logo/travelwai-icon.webp?v=2026-07-26-brand-icon-v2";
      }
    ...
    <div class="day-plan-card-redesigned" data-date="${date}">
      <div class="day-plan-header-redesigned">
        ...
        <div class="day-plan-header-actions">
          ...
          <button type="button" class="day-plan-toggle" onclick="toggleDayPlanRedesigned(\'${date}\')"><span data-interface-icon="chevron-down"></span></button>
        </div>
      </div>
...
function ensureTimePhaseRedesigned(date, phaseIndex) {
  if (!dailyPlans[date]) {
    dailyPlans[date] = { timePhases: [] };
  }

  if (!Array.isArray(dailyPlans[date].timePhases)) {
    dailyPlans[date].timePhases = [];
  }
  ...
  if (!Array.isArray(dailyPlans[date].timePhases[phaseIndex].activities)) {
    dailyPlans[date].timePhases[phaseIndex].activities = [];
  }

  return dailyPlans[date].timePhases[phaseIndex];
}

function updatePhaseNameRedesigned(date, phaseIndex, value) {
  const phase = ensureTimePhaseRedesigned(date, phaseIndex);
  phase.name = value;
}
  ...
  if (dayCard) {
    ...
    if (wasExpanded) {
      ...
      if (newContent && newButton) {
        ...
        newButton.innerHTML = window.TravelwAIIcons?.html ? window.TravelwAIIcons.html("chevron-up") : '<span data-interface-icon="chevron-up"></span>';
      }
    }
  }
}
    ...
    html += `
      <div class="time-phase-block">
        <div class="phase-block-header">
          <h5 data-no-translate>${escapeHtml(phase.name)}</h5>
          ${
            phase.timeRange
              ? `<span class="phase-block-time" data-no-translate>${escapeHtml(
                  phase.timeRange
                )}</span>`
              : ""
          }
        ...
        <ul class="activity-list-detailed">
          ${phase.activities
            ...
            <li class="activity-item-detailed">
              <div class="activity-name-detailed" data-no-translate>${escapeHtml(
                activity.name
              )}</div>
              ${
                activity.description
                  ? `<div class="activity-description-detailed" data-no-translate>${escapeHtml(activity.description)}</div>`
                  : ""
              }
              ${
                activity.estimated_duration
                  ? `<div class="activity-duration-detailed"><small>Thời lượng: <span data-no-translate>${escapeHtml(
                      activity.estimated_duration
                    )}</span></small></div>`
                  : ""
              }
...
function toggleDayPlanRedesigned(date) {
  ...
  if (content.classList.contains("expanded")) {
    ...
    button.innerHTML = window.TravelwAIIcons?.html ? window.TravelwAIIcons.html("chevron-down") : '<span data-interface-icon="chevron-down"></span>';
  } else {
    content.classList.add("expanded");
    button.innerHTML = window.TravelwAIIcons?.html ? window.TravelwAIIcons.html("chevron-up") : '<span data-interface-icon="chevron-up"></span>';
  }
}
      ...
      <div class="preview-time-phase">
        <div class="preview-phase-header">
          <div class="preview-phase-title">
            <span class="preview-phase-name" data-no-translate>${escapeHtml(
              phase.name || `Khung giờ ${index + 1}`
            )}</span>
            ${
              phase.timeRange
                ? `<span class="preview-phase-time" data-no-translate>${escapeHtml(
                    phase.timeRange
                  )}</span>`
                : ""
            }
      ...
      phase.activities.forEach((activity) => {
        html += `
          <div class="preview-activity">
            <div class="preview-activity-name" data-no-translate>${escapeHtml(
              activity.name || "Hoạt động chưa đặt tên"
            )}</div>
            ${
              activity.notes
                ? `<div class="preview-activity-notes" data-no-translate>${escapeHtml(
                    activity.notes
                  )}</div>`
                : ""
            }
