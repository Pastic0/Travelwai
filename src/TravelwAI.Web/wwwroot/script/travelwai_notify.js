(function () {
  ...
  function localizeText(value, callback) {
    const text = String(value || "").trim();
    const language = window.TravelwAILanguage;
    if (language?.get?.() === "en" && typeof language.translateText === "function") {
      language.translateText(text, (translated) => callback(String(translated || text).trim()));
      return;
    }
    callback(text);
  }
  ...
  function notify(message, type, timeout, options) {
    const source = String(message || "").trim();
    if (!source) return;
    ready(() => localizeText(source, (text) => {
      ...
      item.setAttribute("aria-label", `${text}. ${window.TravelwAILanguage?.get?.() === "en" ? "Click to dismiss." : "Bấm để đóng thông báo."}`);
      item.title = window.TravelwAILanguage?.get?.() === "en" ? "Click to dismiss" : "Bấm để đóng";
      ...
      closeTimer = window.setTimeout(close, Number(timeout) > 0 ? Number(timeout) : 3200);
    }));
  }
