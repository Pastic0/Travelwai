...
let selectedTourMediaFiles = [];
let existingTourMedia = [];
let tourMediaPreviewObjectUrls = [];
...
function setupTourPriceInput() {
  const input = document.getElementById("tourPrice");
  if (!input || input.dataset.vndFormattingReady === "true") return;
  input.dataset.vndFormattingReady = "true";
...
function ownerRoleIsCompany(item) {
...
function ownerRoleIsSales(item) {
  const role = normalizeOwnerRole(getValue(item, "owner_role", "ownerRole", "owner_role_name", "ownerRoleName"));
  return role === "sales" || role === "tour sales" || (!role && !ownerRoleIsCompany(item));
}
...
function getTourSalesAccountId(account) {
...
function setAvailableTourSalesAccounts(accounts) {
  availableTourSalesAccounts = (Array.isArray(accounts) ? accounts : [])
    .filter(account => getTourSalesAccountId(account))
...
async function setupTourSalesField(tour) {
  ...
  if (field.tagName === "SELECT") {
    ...
    accounts.forEach(account => {
      const id = getTourSalesAccountId(account);
    ...
    if (!tour && !field.value && accounts.length) {
      field.value = getTourSalesAccountId(accounts[0]);
    }
...
function getOrderBusinessOriginalDeducted(order) {
  return ownerRoleIsCompany(order) ? getOrderOriginalTotal(order) : 0;
}
...
function getOrderSourceKey(order) {
  const ownerId = getOrderTourOwnerId(order);
  return `${ownerRoleIsCompany(order) ? "business" : "sales"}:${ownerId || getOrderTourSalesName(order)}`;
}

function getOrderBusinessReceivedTotal(order) {
  if (ownerRoleIsCompany(order)) return getOrderBusinessTotal(order);
...
function renderOrderTotal(order) {
  ...
  if (roleIsCompany()) return money(getOrderBusinessTotal(order));
...
function filterToursForCurrentPage(tours) {
  ...
  if (!roleIsAdmin()) visibleTours = visibleTours.filter(tour => !tourIsCanceled(tour));
...
function isTourVideoUrl(url) {
  return /\.(mp4|webm|mov)(?:$|[?#])/i.test(String(url || ""));
}

function normalizeTourMediaItem(item) {
  if (!item) return null;
  if (typeof item === "string") {
    const url = item.trim();
    if (!url) return null;
    const video = isTourVideoUrl(url);
  ...
  const video = String(item.type || "").toLowerCase() === "video" || contentType.startsWith("video/") || isTourVideoUrl(url);
...
function parseTourMedia(tour) {
  if (!tour) return [];
  const rawMedia = getValue(tour, "media", "media_items", "mediaItems");
  let media = [];
  if (Array.isArray(rawMedia)) media = rawMedia.map(normalizeTourMediaItem).filter(Boolean);
  else if (typeof rawMedia === "string" && rawMedia.trim()) {
    try { const parsed = JSON.parse(rawMedia); if (Array.isArray(parsed)) media = parsed.map(normalizeTourMediaItem).filter(Boolean); } catch (_) {}
  }
  const legacyValues = [];
  [getValue(tour, "media_urls", "mediaUrls"), getValue(tour, "image_urls", "imageUrls", "images"), getValue(tour, "video_urls", "videoUrls"), getValue(tour, "image", "image_url", "imageUrl")].forEach(raw => {
    if (Array.isArray(raw)) legacyValues.push(...raw);
    else if (typeof raw === "string" && raw.trim()) {
      try { const parsed = JSON.parse(raw); if (Array.isArray(parsed)) legacyValues.push(...parsed); else legacyValues.push(raw); } catch (_) { legacyValues.push(...raw.split(/[\n,|]+/)); }
    }
  });
  legacyValues.map(normalizeTourMediaItem).filter(Boolean).forEach(item => { if (!media.some(current => current.url === item.url)) media.push(item); });
  return media.slice(0, 12);
}

function validateTourMediaFile(file) {
  if (!file) return;
  const type = String(file.type || "");
  if (!type.startsWith("image/") && !type.startsWith("video/")) throw new Error("Chỉ hỗ trợ ảnh hoặc video.");
  if (file.size > 10 * 1024 * 1024) throw new Error("Mỗi tệp tour phải nhỏ hơn 10MB.");
}

function revokeTourMediaPreviewObjectUrls() {
  tourMediaPreviewObjectUrls.forEach(url => URL.revokeObjectURL(url));
  tourMediaPreviewObjectUrls = [];
}

function syncTourMediaHidden() {
  const hidden = document.getElementById("tourMedia");
  if (hidden) hidden.value = JSON.stringify(existingTourMedia);
  const legacyImage = document.getElementById("tourImage");
  if (legacyImage) legacyImage.value = existingTourMedia.find(item => item.type === "image")?.url || "";
}

function tourMediaPreviewMarkup(item, index, source) {
  const media = item.type === "video"
    ? `<video preload="metadata" muted playsinline src="${escapeHtml(item.url)}"></video>`
    : `<img src="${escapeHtml(item.url)}" alt="${escapeHtml(item.name || `Tệp tour ${index + 1}`)}" />`;
  return `<div class="image-attachment-preview-item">${media}<button class="image-attachment-remove" type="button" data-tour-media-source="${source}" data-tour-media-index="${index}" title="Xóa tệp" aria-label="Xóa tệp"><span data-interface-icon="trash-2"></span></button></div>`;
}

function renderTourImagePreview() {
  const preview = document.getElementById("tourImagePreview");
  if (!preview) return;
  revokeTourMediaPreviewObjectUrls();
  const selected = selectedTourMediaFiles.map(file => {
    const url = URL.createObjectURL(file);
    tourMediaPreviewObjectUrls.push(url);
    return normalizeTourMediaItem({ url, name: file.name, contentType: file.type, size: file.size });
  });
  preview.innerHTML = existingTourMedia.map((item, index) => tourMediaPreviewMarkup(item, index, "existing"))
    .concat(selected.map((item, index) => tourMediaPreviewMarkup(item, index, "selected"))).join("");
  preview.querySelectorAll(".image-attachment-remove").forEach(button => {
    button.addEventListener("click", () => {
      const index = Number(button.dataset.tourMediaIndex);
      if (button.dataset.tourMediaSource === "existing") existingTourMedia.splice(index, 1);
      else selectedTourMediaFiles.splice(index, 1);
      syncTourMediaHidden();
...
async function uploadTourMediaFiles(files) {
  const list = Array.from(files || []);
  if (!list.length) return [];
  list.forEach(validateTourMediaFile);
  const prepared = [];
  ...
  const response = await authenticatedFetch("/api/tour-sales/tour-media", { method: "POST", body: formData });
  const result = await readJson(response);
  return (Array.isArray(result.media) ? result.media : []).map(normalizeTourMediaItem).filter(Boolean);
}
...
function filterOrdersForCurrentPage(orders) {
  ...
  return roleIsAdmin() ? source : source.filter(order => canViewOrder(order));
}
...
function revenueRootOrders(type, soldOrders) {
  ...
  if (type === "business") return soldOrders.filter(ownerRoleIsCompany);
...
function getRevenueCompanyName(order) {
  return getOrderTourSalesName(order) || (ownerRoleIsCompany(order) ? "Company" : "Sales");
}

function getRevenueCompanyRoleText(order) {
  return ownerRoleIsCompany(order) ? "Company" : "Sales";
}

function getRevenueCompanyGroups(rootType, orders) {
  ...
  orders.forEach(order => {
    ...
    const current = groups.get(key) || {
      key,
      rootType,
      name: getRevenueCompanyName(order),
      roleText: getRevenueCompanyRoleText(order),
      amount: 0,
      orders: []
    };
...
function renderRevenueCompanyRows(rootType, orders) {
  const groups = getRevenueCompanyGroups(rootType, orders);
...
function renderRevenueAdminTree(orders, netRevenue, companyRevenue) {
  ...
  return rootTypes.map(type => {
    ...
    const childHtml = `<div class="revenue-tree-children revenue-tree-level-2-wrap">${renderRevenueCompanyRows(type, rootOrders)}</div>`;
...
function renderRevenueSingleRoleTree(type, soldOrders, fallbackAmount = 0) {
  ...
  const childHtml = `<div class="revenue-tree-children revenue-tree-level-2-wrap">${renderRevenueCompanyRows(type, soldOrders)}</div>`;
  return revenueTreeRow(key, revenueRootLabel(type), amount, childHtml);
}

function renderRevenueCompanyTree(soldOrders, companyRevenue) {
...
function renderRevenueSourceRows(orders, netRevenue, companyRevenue) {
  ...
  if (roleIsCompany()) return renderRevenueCompanyTree(soldOrders, companyRevenue);
...
async function openTourModal(tour = null) {
  ...
  document.getElementById("tourPrice").value = formatVndInput(tour?.price ?? 0);
  ...
  existingTourMedia = parseTourMedia(tour);
  selectedTourMediaFiles = [];
  syncTourMediaHidden();
...
function editTour(id) {
  ...
  if (!canEditTour(tour)) return showToast("Bạn chỉ được sửa tour của mình.");
  openTourModal(tour);
}
...
async function submitTourForm(event) {
  ...
  const price = parseVndInput(document.getElementById("tourPrice").value);
  if (!Number.isFinite(price) || price < 0) {
    showToast("Giá tour không hợp lệ. Có thể nhập 0, 1000000 hoặc 1.000.000.", "error");
  ...
  const hasNewMedia = selectedTourMediaFiles.length > 0;

  try {
    if (submitButton) {
      submitButton.textContent = hasNewMedia ? "Đang tải tệp..." : "Đang lưu...";
      submitButton.disabled = true;
    }

    const uploadedMedia = await uploadTourMediaFiles(selectedTourMediaFiles);
    const media = existingTourMedia.concat(uploadedMedia).slice(0, 12);
    const imageValue = media.find(item => item.type === "image")?.url || "";
...
async function deleteTour(id) {
  const tour = travelwaiTours.find(t => String(t.id) === String(id));
  if (tour && !canEditTour(tour)) return showToast("Bạn chỉ được xóa tour của mình.");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("tourImageFile")?.addEventListener("change", (event) => {
    ...
    try {
      addedFiles.forEach(validateTourMediaFile);
      const remaining = Math.max(0, 12 - existingTourMedia.length - selectedTourMediaFiles.length);
      selectedTourMediaFiles = selectedTourMediaFiles.concat(addedFiles.slice(0, remaining));
      if (addedFiles.length > remaining) showToast("Mỗi tour tối đa 12 ảnh hoặc video.");
      event.target.value = "";
      renderTourImagePreview();
    } catch (error) {
      event.target.value = "";
      showToast(error.message || "Tệp không hợp lệ.", "error");
    }
  });
  setupTourSearch();
  setupOrderSearch();
  ...
  if (document.body.dataset.page === "business") {
    loadTourSalesPage();
  }
});
